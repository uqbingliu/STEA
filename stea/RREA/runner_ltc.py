...
class Runner():
    def __init__(self, data_dir, output_dir, enhanced=False,
        ...
        self.model, self.get_emb = create_model(batch_size=self.batch_size, dropout_rate=0.30, node_size=self.node_size, rel_size=rel_size,
                                                  ...
                                                  gamma=3,
                                                  node_hidden=50, rel_hidden=50, triple_size=triple_size)
    ...
    def continue_training(self,ite,kg1id,kg2id):
        epoch = 0
        best_perf = None
        ...
        with open(os.path.join(self.output_dir, "training_log.json"), "w+") as file:
            file.write(json.dumps(logs))
    def continue_iterative_training(self,ite,kg1id,kg2id):
        epoch = 0
        best_perf = None
        log_fn = os.path.join(self.output_dir, "training_log.json")
        if os.path.exists(log_fn):
        ...
        revlist1 = []
        revlist2 = []
        for line in open(os.path.join(self.data_dir, f"{kg1id}_entity_id2uri.txt"),'r'):
            e1,e2 = line.split()
            list1.append((int(e1),int(e2)))
            revlist1.append((int(e2),int(e1)))
        for line in open(os.path.join(self.data_dir, f"{kg2id}_entity_id2uri.txt"),'r'):
            e1,e2 = line.split()
            revlist2.append((int(e2),int(e1)))
            list2.append((int(e1),int(e2)))
        revlist1=dict(revlist1)
        revlist2=dict(revlist2)
        list1=dict(list1)
        list2=dict(list2)
        truth=[]
        ...
        train_1=[]
        train_2=[]
        for line in open(os.path.join(self.data_dir, "train_alignment.txt"),'r'):
            e1,e2 = line.split()
            new_train_pair.append([int(e1),int(e2)])
            train_1.append(list1[int(e1)])
            train_2.append(list2[int(e2)])
        # pthrtrue=[1,1,1]
        # pthrfalse=[1,1,1]
        # pthrtrue_inv=[1,1,1]
        # pthrfalse_inv=[1,1,1]
        ...
        # set_1=self.rest_set_1
        # set_2=self.rest_set_2
        # poplist=[]
        # for i, ent in enumerate(set_1):
        #     if ent in new_train_pair[:,0]:
        #         poplist.append(i)
        # poplist=sorted(poplist,reverse=True)
        # for i in poplist:
        #     set_1.pop(i)
        # poplist=[]
        # for i, ent in enumerate(set_2):
        #     if ent in new_train_pair[:,1]:
        #         poplist.append(i)
        # poplist=sorted(poplist,reverse=True)
        # for i in poplist:
        #     set_2.pop(i)       
        ...
        for turn in range(1):
            ...
            # probs=[]
            # for line in open(os.path.join(self.data_dir, "probs"),'r'):
            #     probs.append([int(item) for item in line.split()])
            new=0
            overlap=0
            for a, b in A:
                if B[b][1] == a:
                    # for i,pair in enumerate(self.train_pair):
                    # if pair[0]==self.rest_set_1[a] or pair[1]==self.rest_set_2[b]:
                    st1=self.rest_set_1[a]
                    st2=self.rest_set_2[b]
                    # maxprob=torch.max(improved_prob_mtx[revlist1[st1]],dim=-1)[0].item()
                    # prob=improved_prob_mtx[revlist1[st1]][revlist2[st2]].item()
                    # maxprob_inv=torch.max(improved_prob_mtx_inv.t()[revlist2[st2]],dim=-1)[0].item()
                    # prob_inv=improved_prob_mtx_inv[revlist2[st2]][revlist1[st1]].item()
                    # thrs=[0,0,0]
                    # if truth[st1]==st2:
                    #     if prob>maxprob*0.2:
                    #         pthrtrue[0]+=1
                    ...
                    #                 pthrtrue[2]+=1
                    #                 thrs[2]+=1
                    #     if prob_inv>maxprob_inv*0.2:
                    #         pthrtrue_inv[0]+=1
                    #         thrs[0]+=1
                    #         if prob_inv>maxprob_inv*0.5:
                    #             pthrtrue_inv[1]+=1
                    #             thrs[1]+=1
                    #             if prob_inv>maxprob_inv*0.8:
                    #                 pthrtrue_inv[2]+=1   
                    #                 thrs[2]+=1 
                    #     if thrs[0]==2:
                    #         pthrtrue_bil[0]+=1
                    ...
                    if st1 in d:
                        ...
                        #     for i,pair in enumerate(self.train_pair):
                        #         if pair[0]==st1:
                        #             np.delete(self.train_pair, i)
                        #             if truth[pair[0]]==pair[1]:
                        #                 ptrue+=1
                        #                 # ptrue+=1
                        #             else:
                        #                 pfalse+=1
            ...
            # print("filtered: %d." % reject)
            if len(new_pair)>0:
                self.train_pair = np.concatenate([self.train_pair, np.array(new_pair)], axis=0)
                # keep=len(self.train_pair)
                
            # if len(buf)>0:
            #     self.train_pair = np.concatenate([self.train_pair, np.array(rej_pair)], axis=0)
            for e1, e2 in new_pair:
                if e1 in self.rest_set_1:
                    self.rest_set_1.remove(e1)
        ...
        # if len(o_pairs)>0:
        #     new_train_pair.extend(new_pairs)
        # while True:
        st=sttrue+stfalse
        # noacc=notrue/(notrue+nofalse)
        oacc=otrue/(otrue+ofalse)
        stacc=sttrue/st
        # pacc=ptrue/(ptrue+pfalse)
        newacc=newtrue/(newtrue+newfalse)
        print("self-training acc:",stacc)
        print("overlap acc: " ,oacc)
        # print("self-training replacing acc: " ,noacc)
        # print("replaced paris acc: " ,pacc)
        print("self-training new acc: " ,newacc)
        with open(os.path.join(self.data_dir,"acc", "selftraining_acc.txt"), "a") as file:
            file.write(f"{stacc}\n")
        with open(os.path.join(self.data_dir,"acc", "overlap_acc.txt"), "a") as file:
            file.write(f"{oacc}\n")
        # with open(os.path.join(self.data_dir,"acc", "st_non_acc.txt"), "a") as file:
        #     file.write(f"{noacc}\n")
        # with open(os.path.join(self.data_dir,"acc", "paris_non_acc.txt"), "a") as file:
        #     file.write(f"{pacc}\n")        
        with open(os.path.join(self.data_dir,"acc", "st_new_acc.txt"), "a") as file:
            file.write(f"{newacc}\n")
        with open(os.path.join(self.output_dir, "training_log.json"), "w+") as file:
            file.write(json.dumps(logs))
        if os.path.isfile(os.path.join(self.data_dir, "o_pair.txt")):
            os.remove(os.path.join(self.data_dir, "o_pair.txt"))
        with open(os.path.join(self.data_dir, "o_pair.txt"), "w+") as file:
            for a,b in o_pairs:
                file.write(f"{revlist1[a]}\t{revlist2[b]}\n") 
        # with open(os.path.join(self.data_dir, "train_alignment.txt"), "w+") as file:
        ...
        # pthracc_bil=[0,0,0]
        # for i in range(3):
        #     pthrall=pthrtrue[i]+pthrfalse[i]
        #     pthracc=pthrtrue[i]/pthrall
        #     pthrrat=pthrall/st
        #     pthrall_inv=pthrtrue_inv[i]+pthrfalse_inv[i]
        #     pthracc_inv=pthrtrue_inv[i]/pthrall_inv
        #     pthrrat_inv=pthrall_inv/st
        #     pthrall_bil=pthrtrue_bil[i]+pthrfalse_bil[i]
        #     pthracc_bil=pthrtrue_bil[i]/pthrall_bil
        #     pthrrat_bil=pthrall_bil/st
            
        #     print('self-training with compatibility threshold of',0.2+0.3*i,'num:',pthrall)
        #     print('self-training with compatibility threshold of',0.2+0.3*i,'rat:',pthrrat)
        #     print('self-training with compatibility threshold of',0.2+0.3*i,'acc:',pthracc)
        #     print('self-training with inverse compatibility threshold of',0.2+0.3*i,'num:',pthrall_inv)
        #     print('self-training with inverse compatibility threshold of',0.2+0.3*i,'rat:',pthrrat_inv)
        #     print('self-training with inverse compatibility threshold of',0.2+0.3*i,'acc:',pthracc_inv)
        #     print('self-training with bilateral compatibility threshold of',0.2+0.3*i,'num:',pthrall_bil)
        #     print('self-training with bilateral compatibility threshold of',0.2+0.3*i,'rat:',pthrrat_bil)
        #     print('self-training with bilateral compatibility threshold of',0.2+0.3*i,'acc:',pthracc_bil)
        #     with open(os.path.join(self.data_dir, f"thr_{0.2+0.3*i}_acc.txt"), "a") as file:
        #         file.write(f"{pthracc}\n")
        #     with open(os.path.join(self.data_dir, f"thr_{0.2+0.3*i}_inv_acc.txt"), "a") as file:
        #         file.write(f"{pthracc_inv}\n")    
        #     with open(os.path.join(self.data_dir, f"thr_{0.2+0.3*i}_bil_acc.txt"), "a") as file:
        #         file.write(f"{pthracc_bil}\n")            
            
            
    def save(self, save_metrics=False):
        print('save model into',self.output_dir)
        # save model
        ...
        vec = self.get_embedding()
        vec = vec / np.linalg.norm(vec, axis=-1, keepdims=True)
        with open(os.path.join(self.data_dir, "ent_ids_1"),encoding="utf-8") as file:
            lines = file.read().strip().split("\n")
            ent1_id_list = [int(line.split()[0]) for line in lines]
        with open(os.path.join(self.data_dir, "ent_ids_2"),encoding="utf-8") as file:
            lines = file.read().strip().split("\n")
...
if __name__ == "__main__":
    ...
    if args.enhanced:
        ...
        if args.initial_training == "supervised":
            runner.train()
        elif args.initial_training == "iterative":
            runner.iterative_training()
        else:
            raise Exception("unknown initial training method")

    runner.save(save_metrics=args.neu_save_metrics)
