#!/bin/bash
#SBATCH --job-name=one2one_gcn
#SBATCH --output=wiener_logs/one2one_gcn.out
#SBATCH --error=wiener_logs/one2one_gcn.err
...
for data_name in "zh_en" "ja_en"
do
  for train_percent in 0.01 0.05 0.1 0.2 0.3
#  for train_percent in 0.05 0.1 0.2 0.3
  do
    task="overall_perf_gcnalign2_onetoone_${initial_training}_${train_percent}_seed${seed}"
    ...
    --initial_training=${initial_training} --continue_training=${continue_training} --ea_model=gcnalign --max_train_epoch=${max_train_epoch} --max_continue_epoch=${max_continue_epoch}"
    echo $params
