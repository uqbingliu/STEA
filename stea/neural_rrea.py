...
class RREAModule(NeuralEAModule):
    ...
    def train_model_with_observed_labels(self):
        ...
        if self.conf.py_exe_fn is None:
            print("eval freq: ", self.conf.eval_freq)
            ...
            if self.conf.initial_training in ("supervised", "sup"):
                runner.train()
            elif self.conf.initial_training in ("iterative", "semi"):
    ...
    def train_model_with_observed_n_latent_labels(self,ite):
        ...
        if self.conf.py_exe_fn is None:
            ...
            # runner.train()
            if self.conf.continue_training in ("supervised", "sup"):
                runner.continue_training()
            elif self.conf.continue_training in ("iterative", "semi"):
    ...
    def predict_simi(self):
        ...
        evaluator = Evaluator(device=self.conf.device)
    ...
    def enhance_latent_labels_with_jointdistr_joint_distr_one2one(self, improved_candi_probs: torch.Tensor, improved_candi_probs_inv: torch.Tensor,ratio=0):
        from graph_tool.all import Graph, max_cardinality_matching  # necessary
        print('enhance_latent_labels_with_jointdistr_joint_distr_one2one')
        # neural_sim_mtx=improved_candi_probs.cpu().detach().numpy()
        kg1id, kg2id = self.conf.data_name.split("_")
        kg1_entities = load_alignment(os.path.join(self.conf.data_dir, f"{kg1id}_entity_id2uri.txt"))
        kg2_entities = load_alignment(os.path.join(self.conf.data_dir, f"{kg2id}_entity_id2uri.txt"))
        kg1_newid_to_oldid_map = dict(kg1_entities)
        ...
        x, y = torch.where(improved_candi_probs > ratio)
        potential_aligned_pairs=set(zip(x.cpu().numpy(), y.cpu().numpy()))
        
        k=self.conf.topK
        neighbors = set()
        num = improved_candi_probs.shape[0]
        for i in range(num):
            # rank = np.argpartition(-neural_sim_mtx[i, :], k)
            rank=torch.topk(improved_candi_probs[i, :],k).indices.cpu().numpy()
            pairs = [j for j in itertools.product([i], rank)]
            neighbors |= set(pairs)
        ...
        for x, y in pairs:
            ...
            e = g.add_edge(n1, n2)
            edges.append(e)
            weight_map[g.edge(n1, n2)] = improved_candi_probs[x, y].cpu().item()
        print("graph via graph_tool", g)
        res = max_cardinality_matching(g, heuristic=True, weight=weight_map, minimize=False)
        edge_index = np.where(res.get_array() == 1)[0].tolist()
        curr_labeled_alignment = set()
        ...
        for i, j in curr_labeled_alignment:
            ...
            if i in labeled_alignment_dict.keys():
                pre_j = labeled_alignment_dict.get(i)
                pre_sim = improved_candi_probs[i, pre_j].cpu().item()
                new_sim = improved_candi_probs[i, j].cpu().item()
                if new_sim >= pre_sim:
                    labeled_alignment_dict[i] = j
            else:
                labeled_alignment_dict[i] = j
        ...
        for j, i_set in labeled_alignment_dict.items():
            if len(i_set) == 1:
                ...
                max_sim = -10
                for i in i_set:
                    if improved_candi_probs[i, j].cpu().item() > max_sim:
                        max_sim = improved_candi_probs[i, j].cpu().item()
                        max_i = i
                new_alignment.append([max_i, j])
                if max_i==j:
                    ptrue+=1
    ...
    def enhance_latent_labels_with_jointdistr(self, improved_candi_probs: torch.Tensor, improved_candi_probs_inv: torch.Tensor,
        ...
        if improved_candi_probs is None:
            ...
            if conf.joint_distr_one2one == "True":
                print('============================True===============================')
                # self.enhance_latent_labels_with_jointdistr_joint_distr_one2one(improved_candi_probs=improved_candi_probs, improved_candi_probs_inv=improved_candi_probs_inv,ratio=conf.joint_distr_thr)
