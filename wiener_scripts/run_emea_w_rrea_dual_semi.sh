#!/bin/bash
#SBATCH --job-name=dual_rrea_semi
#SBATCH --output=wiener_logs/dual_rrea_semi.out
#SBATCH --error=wiener_logs/dual_rrea_semi.err
...
#  for train_percent in 0.01
  do
    task="overall_perf_rrea_dual_${initial_training}_${train_percent}_seed${seed}"
    ...
    --tf_device=${tf_device} --device=${device} --py_exe_fn=${py_exe_full_fn}
